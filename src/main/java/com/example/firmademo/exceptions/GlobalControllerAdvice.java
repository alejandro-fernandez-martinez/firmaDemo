...
public class GlobalControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleKeysNotFound(
        KeysNotFoundException ex, WebRequest request) {
    ...
    @ExceptionHandler(SignatureVerificationFailed.class)
    public ResponseEntity<?> handleSignatureVerificationsFailed(
            SignatureVerificationFailed ex, WebRequest request) {
        ExcepcionBody body = new ExcepcionBody(LocalDateTime.now(),
                HttpStatus.NOT_FOUND, ex.getMessage(),
                ((ServletWebRequest) request).getRequest().getRequestURI());
        return new ResponseEntity<Object>(body, HttpStatus.NOT_FOUND);
